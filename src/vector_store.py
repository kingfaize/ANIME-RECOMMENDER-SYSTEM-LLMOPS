...
class VectorStoreBuilder:
    def __init__(self, csv_path: str, persist_dir: str = "chroma_db", embedding_key: str = None):
        self.csv_path = csv_path
        self.persist_dir = persist_dir
        if embedding_key:
            self.embedding = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2", huggingfacehub_api_token=embedding_key)
        else:
